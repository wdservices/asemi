...
function assertQty(qty: unknown): number {
  ...
  if (!Number.isInteger(n) || n < 1 || n > 1_000_000) {
    throw new HttpsError("invalid-argument", "Quantity must be an integer between 1 and 1,000,000.");
  }
  return n;
}
...
export const generatebatchpaid = onCall(
  ...
  async (req) => {
    ...
    if (company.status !== "approved") {
      throw new HttpsError("failed-precondition", "Company must be approved before generating paid batches.");
    }
    ...
    if (resumeBatchId) {
      ...
      await db.collection("batches").doc(batchId).update({ status: "generating", failedAt: FieldValue.delete() });
    } else {
      ...
      if (quote.requiresQuote) {
        throw new HttpsError("failed-precondition", "Volume exceeds 1,000,000 lifetime codes — contact sales for a custom quote.");
      }
      price = roundMoney(quote.price);
      currency = quote.currency;
      free = quote.free;
    ...
    try {
      ...
      try {
        await db.runTransaction(async (tx) => {
          ...
          tx.set(wRef, {
            creditBalance: roundMoney(balance + price!),
            lifetimeSpent: roundMoney(Math.max(0, Number(wSnap.data()?.lifetimeSpent || 0) - price!)),
            updatedAt: FieldValue.serverTimestamp(),
          }, { merge: true });
          tx.update(cRef, {
      ...
      throw new HttpsError("internal", `Code generation failed partway and was refunded. Resume with resumeBatchId=${batchId}.`);
    }

    return { batchId: batchId!, batchNumber: batchNumber!, quantity: qty!, price, currency, free, status: "ready" };
  },
);
...
export const verifycode = onCall(async (req) => {
  ...
  const scansSnap = await db.collection("codes").doc(codeDoc.id).collection("scans").select("browserToken", "city", "scannedAt").limit(5000).get();
  const now = Date.now();
  ...
  return {
    ...
    batch: {
      ...
      produced_at: batch.createdAt instanceof Timestamp ? batch.createdAt.toDate().toISOString() : null,
...
export const topupwallet = onCall(async (req) => {
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    const currency = wSnap.exists
      ? String(wSnap.data()?.currency || regionFor((cSnap.data() as CompanyDoc).countryCode).currency)
...
export const exportbatch = onCall({ timeoutSeconds: 540, memory: "1GiB" }, async (req) => {
  ...
  let last: FirebaseFirestore.QueryDocumentSnapshot | undefined;
  for (;;) {
    let q = db.collection("codes").where("batchId", "==", batchId).select("codeString").orderBy("codeString").limit(1000);
    if (last) q = q.startAfter(last) as typeof q;
    const snap = await q.get();
    if (snap.empty) break;
  ...
  for (const s of codeStrings) {
    csvLines.push(`${s},${process.env.APP_URL || "https://asemi-c14ac.web.app"}/v/${encodeURIComponent(s)}`);
  }
...
async function buildLabelPdf(codeStrings: string[], batchLabel: string): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: "A4", margin: 36, info: { Title: `Asemi labels — ${batchLabel}` } });
    const chunks: Buffer[] = [];
